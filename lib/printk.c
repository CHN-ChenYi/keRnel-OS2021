...
void putc(char c) { sbi_ecall(SBI_PUTCHAR, 0, c, 0, 0, 0, 0, 0); }
...
int log(const LogLevel level, const char* fmt, ...) {
  switch (level) {
    case LOG_LEVEL_DEBUG:
      printk("[" ANSI_COLOR_MAGENTA "DEBUG" ANSI_COLOR_RESET " ] ");
      break;
    case LOG_LEVEL_INFO:
      printk("[ " ANSI_COLOR_BLUE "INFO" ANSI_COLOR_RESET " ] ");
      break;
    case LOG_LEVEL_OK:
      printk("[  " ANSI_COLOR_GREEN "OK" ANSI_COLOR_RESET "  ] ");
      break;
    case LOG_LEVEL_WARN:
      printk("[ " ANSI_COLOR_YELLOW "WARN" ANSI_COLOR_RESET " ] ");
      break;
    case LOG_LEVEL_ERROR:
      printk("[" ANSI_COLOR_RED "ERROR" ANSI_COLOR_RESET " ] ");
      break;
    case LOG_LEVEL_FATAL:
      printk("[" ANSI_COLOR_RED "FATAL" ANSI_COLOR_RESET " ] ");
      break;
  }
  int res = 0;
  va_list vl;
  va_start(vl, fmt);
  res = vprintfmt(putc, fmt, vl);
  va_end(vl);
  return res;
}
